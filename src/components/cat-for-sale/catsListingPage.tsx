/**
 * CatsScreen.tsx
 * React Native conversion of the CatsPage web component.
 *
 * FILTERS SIMPLIFIED:
 *   Only City, Price (budget) and Gender remain.
 *   Breed search, State picker, Sort By, Pet Features and Popular Breeds
 *   have all been removed per request.
 *
 * PAGE STRUCTURE:
 *   1. Filter (City / Price / Gender)
 *   2. Cats (grid list of ads)
 *
 * Required packages (install via npm/yarn):
 *   react-native-vector-icons
 *   @react-navigation/native (for navigation.navigate)
 *
 * NOTE: `useAdStore` and `statesWithCatCities` are assumed to already work
 * in a React Native context (no `window`/`document` dependency inside them).
 * `sessionStorage` (web-only) has been replaced with passing the pet object
 * directly as a navigation param.
 */
...
import { statesWithCatCities } from "./data";

const ITEMS_PER_PAGE = 12;
const PRIMARY = "#7C3AED"; // swap for your `--color-primary` value
...
// Local slider avoids requiring @react-native-community/slider in this screen.
const Slider = ({
  minimumValue,
  maximumValue,
  value,
...
const ALL_CITIES: string[] = Array.from(
  new Set(Object.values(statesWithCatCities).flat() as string[])
).sort();
...
const router = useRouter();
  ...
  // AdsStore exposes the generic approved-ads method; keep the cat-specific
  // alias local to this screen so the store's public type is respected.
 const { getApprovedAds: fetchApprovedAds } = useAdStore();
  const getApprovedAds = (p0: string, page: number, limit: number) =>
    fetchApprovedAds("cats", page, limit);
...
useEffect(() => {
  const fetchApprovedCatAds = async () => {
    setLoading(true);
    try {
     const result = await getApprovedAds("cats", currentPage, ITEMS_PER_PAGE);
...
const handleViewPet = (pet: any) => {
  ...
  console.log("Navigating to pet details slug:", petSlug, "with ID:", petId);
  ...
  const handleWhatsApp = (number: string) => {
    if (!number) return;
    const text = encodeURIComponent("Hi, I saw your ad, I am interested");
  ...
  const renderCatCard = ({ item: pet }: { item: any }) => (
